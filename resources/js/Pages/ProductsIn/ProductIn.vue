...
const totalTax = () => {
  ...
  let stock: number = form.stock || 0.0;
  let cost: number = (form.cost || 0.0) * 100;
  let price: number = (form.price || 0.0) * 100;
    ...
    <div class="max-w-275 mx-auto max-h-[85vh] overflow-y-auto">
      <form v-if="showForm" class="p-5 rounded-md bg-blue-300" @submit.prevent="submit">
        ...
        <div class="mt-4 max-w-2xl">
          ...
          <div class="flex space-x-5">
            <div class="relative flex-1">
              <TextInput maxLength="50" class="w-3/4" v-model="form.product_name" name="product" />
            </div>
          </div>
        </div>
        ...
        <div class="mt-4 grid grid-cols-3 gap-3 overflow-hidden">
          ...
          <!-- Precio Espcial -->
          <div>
            <InputLabel for="minPrice" value="Precio Especial" />
            <Money
              class="inputGeneral w-full"
              @input="totalTax"
              v-bind="moneyConfig"
          ...
          <!-- Precio Special -->
          <div>
            <InputLabel for="minPrice" value="Precio Minímo" />
            <Money
              class="inputGeneral w-full"
              @input="totalTax"
              v-bind="moneyConfig"
          ...
          >
            <legend>Tributario</legend>

            <div class="flex-auto">
              <InputLabel for="tax_rate" value="ITBIS %" />
              <span class="span-white"> {{ form.tax_rate || 0 }} % </span>
            </div>

            <!-- Impuesto -->
            <div class="flex-auto">
              <InputLabel for="tax" value="ITBIS * 1 " />
            ...
            <!-- Precio sin impuesto -->
            <div class="flex-auto">
              <InputLabel for="price-no-tax" value="Precio - ITBIS * 1" />
              <span class="span-white">
            ...
            <!-- Impuesto -->
            <div class="flex-auto">
              <InputLabel for="tax-aomount" value="Total del impuesto" />
              <span class="span-white">
            ...
            <!-- Decuento -->
            <div class="flex-auto">
              <InputLabel for="discount" value="Total Ingresado" />
              <span class="span-white">
            ...
            <!-- Beneficio -->
            <div class="flex-auto">
              <InputLabel for="benefit" value="Beneficio * 1" />
              <span class="span-white">
      ...
      <div class="mt-5 bg-blue-300 p-5 rounded-md">
        <div class="flex justify-between items-center">
          ...
          <h3 class="text-3xl font-bold">Productos</h3>
        </div>

        <div class="max-h-[600px] overflow-y-auto">
          <!--                Datos de los productos para la entrada    -->
          <table class="table-fixed mt-3 styleTable w-full">
            <thead>
              <tr>
              ...
              <tr v-for="item in propsW.products.data">
                ...
                <td>
                  <i
                    @click="edit(item.id)"
                    title="Entrada"
                    class="icon-efect fa-solid fa-dolly"
                  ></i>
                </td>
              </tr>
            </tbody>
      ...
      <FloatBox header="Suplidores" v-if="false">
        <FRegister class="w-full" />
      </FloatBox>
    </div>
  </AppLayout>
</template>
