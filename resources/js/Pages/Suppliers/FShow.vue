    ...
    <div class="mt-3">
      <div class="flex justify-between items-center">
        <form @submit.prevent="search">
          <FormSearch v-model:per-page="form.per_page" v-model:search.number="form.search" />
        </form>

        <i
          @click="download"
          title="Descargar CSV"
          class="fa-solid fa-file-csv text-gray-50 text-[3rem]"
        ></i>

        <h3 class="text-3xl font-bold text-center">Suplidores</h3>
      </div>
      <!--                Datos de los proveedores-->
      <table class="styleTable mt-3 w-full">
        <thead>
          <tr>
            <th @dblclick="field('company_name')" class="">
              <i v-show="form.field === 'company_name'" class="fa-solid fa-arrow-down"></i>
              Empresa
            </th>
            <th @dblclick="field('contact')" class="">
              <i v-show="form.field === 'contact'" class="fa-solid fa-arrow-down"></i>
              Contacto
            </th>
            <th @dblclick="field('phone')" class="">
              <i v-show="form.field === 'phone'" class="fa-solid fa-arrow-down"></i>
              Teléfono
            </th>
            <th @dblclick="field('email')" class="">
              <i v-show="form.field === 'email'" class="fa-solid fa-arrow-down"></i>
              Correo
            </th>
            <th class="">Act</th>
          </tr>
        ...
        <tbody>
          <tr v-for="item in propsW.suppliers.data">
            ...
            <td class="space-x-2">
              <i @click="emit('seeSupplier', item)" class="icon-efect fa-regular fa-eye"></i>
              <i
                @click="edit(item)"
                title="Editar"
                class="icon-efect fa-solid fa-pen-to-square"
              ></i>
              <i
                @click="destroy(item)"
                title="Eliminar"
                class="ml-3 icon-efect fa-solid fa-trash"
              ></i>
            </td>
          </tr>
        </tbody>
