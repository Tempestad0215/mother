...
<template>
  ...
  <AppLayout>
    <template #header>
      ...
      <TabLink :href="route('sequence.create')"> Correlativos </TabLink>
      <TabLink active :href="route('aco.index')"> Cuentas </TabLink>
      <TabLink :href="route('wh.index')"> Almacen </TabLink>
    </template>

    <!--        Contenido de la vantana-->
    <div class="max-w-[70rem] mx-auto">
      <form @submit.prevent="submit" class="grid grid-cols-3 gap-3 fondo p-5 rounded-md">
        <h3 class="title text-center col-span-full">Cuentas Contables</h3>
        <!--                codigo-->
        <div>
          <label for="code">Codigo</label>
          <TextInput class="w-full" v-model="form.code" />
        ...
        <div>
          ...
          <select class="inputGeneral py-1 w-full" v-model="form.type">
        ...
        <div class="col-end-4 text-right">
          <PrimaryButton> Registar </PrimaryButton>
        </div>
      </form>

      <!--            Cuentas registrada -->
      <div class="mt-3 p-5 fondo rounded-md">
        <h3 class="title text-center">Listado de Cuentas</h3>
        <table class="styleTable table-auto w-full mt-3">
          ...
          <tbody>
            <tr v-for="(item, index) in propsW.aco" :key="index">
              <td>{{ item.code }}</td>
              <td>{{ item.name }}</td>
              <td>{{ item.type }}</td>
              <td class="space-x-3">
                <i @click="edit(item)" class="icon-efect fa-solid fa-file-pen"></i>
                <i @click="destroy(item)" class="icon-efect fa-solid fa-trash-can"></i>
              </td>
            </tr>
          </tbody>
        </table>
