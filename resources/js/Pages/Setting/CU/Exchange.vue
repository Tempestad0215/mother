...
const days = () => {
  ...
  axios
    .get(route('currency.get.exchange', { month: form.month, year: form.year }))
    .then((res) => {
...
const submit = () => {
  axios
    .post(route('currency.exchange.store'), form)
    .then((res) => {
...
<template>
  <form @submit.prevent="submit" class="bg-blue-300 p-5 rounded-md max-h-[40rem] overflow-y-auto">
    <div class="pt-3 flex justify-between">
      <select v-model="form.month" class="inputGeneral py-1" @change="days">
        <option v-for="(item, index) in month" :key="index" :value="index + 1">
          {{ item.name }}
        </option>
      </select>

      <select v-model="form.year" class="inputGeneral py-1" @change="days">
        <option v-for="(item, index) in getYear()" :key="index" :value="item">
          {{ item }}
        </option>
      </select>
    </div>
    <table class="styleTable table-auto w-full mt-3">
      ...
      <tbody>
        <tr v-for="(item, index) in form.rate_info" :key="index">
          <td>{{ item.day }}</td>
          <td>
            <Money
