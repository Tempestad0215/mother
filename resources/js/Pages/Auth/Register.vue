...
<template>
  ...
  <AppLayout>
    <template #header>
      <TabLink :active="true" :href="route('register')"> Registrar </TabLink>
    </template>
    <div class="mx-auto max-w-[70rem]">
      ...
      >
        <h3 class="text-3xl font-bold text-center col-span-full">Registro de Usuario</h3>
        ...
        <div v-if="form.update" class="flex-1">
        ...
        <div>
          <label class="block" for="role">Rol</label>
          <select v-model="form.role" class="inputGeneral py-1 w-full" name="role" id="role">
            <option
      ...
      <div class="rounded-md p-5 bg-blue-300 mt-3">
        ...
        <table class="w-full table-auto styleTable mt-3">
          ...
          <tbody>
            <tr class="" v-for="(item, index) in users.data" :key="index">
              <td>{{ item.id }}</td>
              <td>{{ item.name }}</td>
              <td>{{ item.email }}</td>
              <td class="uppercase">{{ item.role }}</td>
              <td class="space-x-4">
                <i @click="edit(item)" class="icon-efect fa-solid fa-pen-to-square"></i>

                <i @click="destroy(item)" class="icon-efect fa-solid fa-trash"></i>
              </td>
            </tr>
          </tbody>
        </table>
