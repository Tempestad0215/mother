    ...
    <Card>
      ...
      <template #content>
        <DataTable :value="propsW.purchases">
          ...
          <Column
            header="Sub Total"
            :field="(data: PurchaseSupplierI) => `${getMoney(data.sub_total)}`"
          />
      ...
      <Card>
        ...
        <template #content>
          ...
          <DataTable :value="purchaseSelected?.items">
            ...
            <Column
              header="Cantidad"
              :field="(data: PurchaseItemI) => `${getMoney(data.quantity)}`"
            />
            <Column header="Itbis" :field="(data: PurchaseItemI) => `${data.tax_rate} %`" />
            <Column
              header="Descuento"
              :field="(data: PurchaseItemI) => `${getMoney(data.discount)}`"
            />
            ...
            <template #footer>
              <div class="float-right">
                ...
                <p class="text-white bg-blue-800 rounded-md px-6 py-1">
                  Total : {{ getMoney(purchaseSelected?.amount) }}
                </p>
              </div>
