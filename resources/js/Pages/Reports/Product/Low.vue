...
<template>
  ...
  <AppLayout>
    ...
    <div class="bg-gray-200 p-5 rounded-md">
      <i @click="back()" class="icon-efect float-left fa-solid fa-circle-arrow-left"> </i>
      <h3 class="text-3xl font-bold text-center mb-5">Producto < 10 Disponible</h3>
      <table class="w-full mt-5">
        ...
        <tbody>
          <tr class="odd:bg-gray-400" v-for="(item, index) in propsW.products" :key="index">
            <td>{{ item.code }}</td>
            <td>{{ item.name }}</td>
            <td>{{ item.stock }}</td>
            <td>{{ item.reserved }}</td>
