...
<template>
  ...
  <AppLayout>
    <template #header>
      ...
      <TabLink :active="true" :href="route('sale.close')"> Cierre </TabLink>
    </template>
    <div class="p-5 fondo rounded-md">
      <div>
        <h3 class="title text-center">Crear Cierre De Ventas</h3>
        <form class="grid grid-cols-2 gap-3 items-end" @submit.prevent="submit">
          <div>
            <label class="block" for="User">Usuarios</label>
            <select v-model="form.user" class="inputGeneral py-1" name="use" id="user">
              <option :value="0">--Selecione Usuario</option>
              <option v-for="(item, index) in propsW.users" :key="index" :value="item.id">
                {{ item.name }}
              </option>
            </select>
          </div>
          ...
          <div class="mt-3 text-right">
            <PrimaryButton :disabled="processing" @click="submit"> Crear Cierre </PrimaryButton>
          </div>
        </form>
        ...
        <div class="grid grid-cols-3 gap-3 mt-3">
          ...
          <div>
            <InputLabel for="amount" value="Beneficio :" />
            <span class="inline-block bg-white px-3 py-1 rounded-md">
              {{ getMoney(reportSale?.benefits) }}
            </span>
          </div>
          <div>
            <InputLabel for="amount" value="Descuento aplicado :" />
            <span class="inline-block bg-white px-3 py-1 rounded-md">
