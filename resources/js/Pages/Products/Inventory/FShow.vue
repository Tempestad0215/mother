...
const search = () => {
  router.get(
    `/product/entry?search=${formSearch.search}&perPage=${formSearch.per_page}`,
    {},
    {
      preserveState: true,
      preserveScroll: true,
...
<template>
  <div class="fondo mt-3 p-3 rounded-md">
    <form @submit.prevent="search">
      <FormSearch v-model:search="formSearch.search" v-model:total="formSearch.per_page" />
    </form>
    <table class="styleTable w-full mt-2">
      ...
      <tbody>
        <tr v-for="(item, index) in propsW.entries.data" :key="index">
