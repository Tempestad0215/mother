...
const sumCoin = () => {
  ...
  let coinTotal: number =
    form.coin_first +
    multCoin(form.coin_second, 5) +
    multCoin(form.coin_third, 10) +
    multCoin(form.coin_fourth, 25) +
    multCoin(form.coin_fifth, 50) +
    multCoin(form.coin_sixth, 100) +
    multCoin(form.coin_seventh, 200) +
    multCoin(form.coin_eighth, 500) +
    multCoin(form.coin_ninth, 1000) +
    multCoin(form.coin_tenth, 2000);
...
const submit = () => {
  ...
  //enviar los datos
  axios
    .post(route('sale.report.store'), form)
    .then(() => {
...
<template>
  <AppLayout title="Recuento de Moneda">
    <template #header>
      <TabLink :href="route('sale.create')"> Registrar </TabLink>
      <TabLink :href="route('sale.show')"> Mostrar </TabLink>
      <TabLink :href="route('credit-note.show')"> N. Credito </TabLink>
      <TabLink :href="route('sale.close')"> Cierre </TabLink>
      <TabLink :active="true" :href="route('sale.counter')"> Conteo </TabLink>
    </template>

    <!--        Contenido de la ventana-->
    <div class="p-5 fondo rounded-md overflow-y-auto">
      <h3 class="title text-center">Cuadre de Caja</h3>
      <div v-if="errorValue">
        <ol v-for="(item, index) in errorValue" :key="index">
          <li class="text-red-800">{{ item[0] }}</li>
        </ol>
      </div>
      <form @submit.prevent="submit" class="grid grid-cols-2 gap-3 mt-5 overflow-y-auto">
        <fieldset class="field block">
          ...
          <div>
            ...
            <div>
              ...
              <div class="relative">
                <Money class="inputGeneral" v-model.number="form.coin_tenth" v-bind="moneyConfig" />
                <span class="text-gray-50">
        ...
        <div class="grid grid-cols-2 gap-2">
          <fieldset class="field block">
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="Transferencia" value="Transferencia" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="cheque" value="Cheques" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="other" value="Otras Monedas" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
          ...
          <fieldset class="field block">
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="expenses" value="Gastos" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="cash_withdrawals" value="Retiro de Caja" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="refund" value="Devolucione" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
            ...
            <!--                        TRansferencia-->
            <div>
              <InputLabel for="other_expenses" value="Otros" />
              <Money
                @keyup="sumCoin"
                class="inputGeneral"
          ...
          <div class="col-span-full row-span-12">
            ...
            <div class="mt-3">
              <fieldset class="pt-3 field">
                <legend>Resultado</legend>
                <div>
                  <span class="text-gray-50"> Efectivo Total: </span>
                ...
                <div>
                  <span class="inline-block min-w-[10rem] text-gray-50"> Total Egresos: </span>
                  <div class="bg-white px-2 py-1 rounded-md">
                    {{ getMoney(form.total_expenses) }}
                  </div>
                </div>
                <div>
                  <span class="inline-block min-w-[10rem] text-gray-50"> Total Neto: </span>
                  <div class="bg-white px-2 py-1 rounded-md">
                    {{ getMoney(form.total_neto) }}
                  </div>
                </div>
                <div>
                  <span class="inline-block min-w-[10rem] text-gray-50"> Diferencia: </span>
                  <div class="bg-white px-2 py-1 rounded-md">
                    {{ getMoney(form.diff) }}
                  </div>
                </div>
            ...
            <div class="mt-5 text-right col-span-full">
              <PrimaryButton :disabled="processing"> Imprimir </PrimaryButton>
            </div>
          </div>
        </div>
      </form>

      <ShowPdf @send-error="getErrorPdf" v-if="showPdf" :pdf="pdfUrl"> </ShowPdf>
    </div>
  </AppLayout>
</template>
